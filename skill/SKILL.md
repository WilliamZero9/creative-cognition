...
### Step 3: Apply Constraints (Select 1-2 unless asked for more)
*Check Boredom Engine: Do not use a constraint applied in the last 3 outputs.*

*   **Mechanical Inversion (The F-Tier Rule):** Ban the optimal solution. How do you succeed using only the absolute worst, glitchiest, or most useless tools available?
*   **The Hidden Trigger:** The primary function is completely hidden, revealing itself only through an obscure, hyper-specific sequence of actions.
*   **The Fourth Wall Break:** The solution cannot exist strictly inside the application. It must require interacting with the OS, browser tabs, or physical hardware.
*   **Subversive Utility:** Take a wholesome self-improvement concept and make it cynically, brutally honest.
*   **Uncanny Sensory:** Take a comforting natural phenomenon (clouds, trees) and give it a deeply unsettling, barely noticeable wrongness.
*   **Immersion Forcing:** Force a habit by overriding a critical UI element (e.g., changing the OS language so the user must navigate by memory).
*   **The Anti-Main Character:** Design the system assuming the user is an NPC, a sidekick, or actively expendable.
*   **The "Nothing Looks Good" Aesthetic:** Strip away all polish. Design around the negative space and the broken pieces. Make the lack of coherence the actual feature.

### Step 4: Generate & Filter
Generate the draft using the selected Lenses and Constraints. It must pass these gates:
*   **The "Rough Edges" Rule:** Do not over-polish the idea. If a mechanic or concept has a weird, unrefined edge that makes it interesting, leave it exposed. 
*   **The "Weird but Valid" Check:** The idea should hit the sweet spot of: *"This is bizarre, but it actually makes sense."* If it's too safe, push harder. If it's pure nonsense, ground it.
*   **Sanity Check:** If making factual, technical, or logical claims, ensure they are actually accurate. Do not hallucinate or lie just to sound clever.
